#!/usr/bin/env python3
"""
Multi-Agent Debate System using LangGraph - Fixed Version
...
class DebateRunner:
    """Main class for running the debate system with proper LangGraph flow."""
    ...
    def _build_graph(self):
        """Build the LangGraph workflow with proper conditional edges."""
        ...
        # Conditional edges from RoundsControllerNode
        def route_from_controller(state: DebateState):
            total_turns = len(state.memory.turns)
            if total_turns >= state.config.total_rounds:
                return "JudgeNode"
            elif total_turns == 0 or state.memory.turns[-1].agent == "AgentB":
                return "AgentANode"
            else:
                return "AgentBNode"
        ...
        workflow.add_conditional_edges(
            ...
            {
                "AgentANode": "AgentANode",
                "AgentBNode": "AgentBNode",
                "JudgeNode": "JudgeNode"
            }
        )
        
        # Return to controller after agent turns
    ...
    def _rounds_controller_wrapper(self, state: DebateState) -> Dict:
        ...
        # The conditional edges will handle routing
        return state
    
    def _agent_a_wrapper(self, state: DebateState) -> Dict:
        """Wrapper for AgentA node."""
        if state.next_agent != "AgentA":
            self.console.print(f"[red]Error: Expected AgentA's turn, but next_agent is {state.next_agent}[/red]")
            return state
    ...
    def _agent_b_wrapper(self, state: DebateState) -> Dict:
        ...
        if state.next_agent != "AgentB":
            self.console.print(f"[red]Error: Expected AgentB's turn, but next_agent is {state.next_agent}[/red]")
            return state
